...
sim_files <- list.files(pattern = ".*.fastq.gz$", path = "fast", full.names = TRUE)
...
res <- bplapply(sim_files, function(file, SSTRING_RANDOM_400){
    ...
    for (max_snps in c(100, 200, 300, 400)){
        ...
        for (read_i in c(0, 1, 2, 3, 4, 5, 6, 7)){
            ...
            toc()
            write.csv(tt, paste0("results/random_400/fast_", id, "_", max_snps, "_", read_i, ".csv"), row.names = FALSE)
            }
    }
}, SSTRING_RANDOM_400 = SSTRING_RANDOM_400, BPPARAM=bp)

res <- bplapply(sim_files, function(file, SSTRING_MINSNPS_200, PREV_RESULT_FILE){
    ...
    for (n_sets in c(1, 2, 3, 4, 5, 30, 79, 147, 200)){
        ...
        for (read_i in c(0, 1, 2, 3, 4, 5, 6, 7)){
            ...
            tt$max_reads_used <- rep( (1000*(read_i+1)), nrow(tt))
            toc()
            write.csv(tt, paste0("results/non_random_200/fast_", id, "_", n_sets, "_", read_i, ".csv"), row.names = FALSE)
            }
    }
}, SSTRING_MINSNPS_200 = SSTRING_MINSNPS_200, PREV_RESULT_FILE = PREV_RESULT_FILE, BPPARAM=bp)
...
res <- bplapply(sim_files, function(file, GENE_SEARCH_SEQUENCE){
    library(minSNPs)
    library(BiocParallel)
    set.seed(42)
    ...
    for (max_sequence in c(100, 200, 300, 400, 500, 600, 700, 800, 900, "all")){
        ...
        for (read_i in c(0, 1, 2, 3, 4, 5, 6, 7)){
            ...
            write.csv(tt, paste0("gene/sim_", file, "_", max_sequence, "_", read_i, ".csv"), row.names = FALSE)
        }
    }
}, GENE_SEARCH_SEQUENCE = GENE_SEARCH_SEQUENCE, BPPARAM=bp)
